...
$SECRET_SALT = $_ENV['SIMPLESAMLPHP_SECRET_SALT'] ?? 'secretsalt';
$ADMIN_PASSWORD = $_ENV['SIMPLESAMLPHP_ADMIN_PASSWORD'] ?? 'admin';
$CONTACT_NAME = $_ENV['SIMPLESAMLPHP_CONTACT_NAME'] ?? 'UBC LT Hub';
$CONTACT_EMAIL = $_ENV['SIMPLESAMLPHP_CONTACT_EMAIL'] ?? 'lt.hub@ubc.ca';

if (!isset($_ENV['SIMPLESAMLPHP_DEV']) && $SECRET_SALT == 'secretsalt') {
    exit("Set env var SIMPLESAMLPHP_SECRET_SALT for non-dev environments. If this is a dev enviroment, set SIMPLESAMLPHP_DEV to skip this error.");
}
if (!isset($_ENV['SIMPLESAMLPHP_DEV']) && $ADMIN_PASSWORD == 'admin') {
    exit("Set env var SIMPLESAMLPHP_ADMIN_PASSWORD for non-dev environments. If this is a dev enviroment, set SIMPLESAMLPHP_DEV to skip this error.");
}
if (!isset($_ENV['SIMPLESAMLPHP_MEMCACHED_SERVER'])) {
    exit('Set env var SIMPLESAMLPHP_MEMCACHED_SERVER to a memcache host.');
}
if (!isset($_ENV['SIMPLESAMLPHP_TRUSTED_DOMAIN'])) {
...
$config = [
    ...
    */
     ...
     */
    'trusted.url.domains' => [$_ENV['SIMPLESAMLPHP_TRUSTED_DOMAIN']],
    ...
    'module.enable' => [
        'exampleauth' => false,
        'core' => true,
        'admin' => isset($_ENV['SIMPLESAMLPHP_DEV']),
        'saml' => true,
        'cron' => true,
        'metarefresh' => true,
    ],
     ...
     */
    'session.cookie.name' => 'SimpleSAMLSessionID-wiki-sp',
     ...
     */
    'session.phpsession.cookiename' => 'SimpleSAML-wiki-sp',
     ...
     */
    'session.authtoken.cookiename' => 'SimpleSAMLAuthToken-wiki-sp',
    ...
    'memcache_store.servers' => [
        [
            ['hostname' => $_ENV['SIMPLESAMLPHP_MEMCACHED_SERVER']],
        ],
    ],
     ...
     */
    'memcache_store.prefix' => 'simpleSAMLphp-wiki-sp',
     ...
     */
    'metadata.sources' => [
        ['type' => 'flatfile', 'directory' => 'metadata/metarefresh-ubc'],
        //['type' => 'xml', 'file' => 'metadata/metarefresh-ubc/shibboleth'],
    ],
