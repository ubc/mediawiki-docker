...
abstract class SkinVector extends SkinMustache {
	...
	private function getUserLinksTemplateData( $menuData, $user ): array {
		...
		if ( $isAnon || $isTempUser ) {
            ...
            # TODO: remove customization not after 1.40+
            # ubc custom: remove user links menu for unsigned in users the
            # login button is in overflow and not allowed to collapse, so the
            # login button insider user links is not needed
            $userMenuData = [];
		} else {
